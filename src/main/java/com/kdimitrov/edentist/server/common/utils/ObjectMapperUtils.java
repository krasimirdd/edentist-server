...
public class ObjectMapperUtils {
    ...
    static {
        ...
        PropertyMap<Appointment, AppointmentDto> propertyMap = new PropertyMap<Appointment, AppointmentDto>() {
            protected void configure() {
                map().setDoctor(source.getDoctor());
                map().setPatient(source.getPatient());
                map().setService(source.getService());
            }
        };
        PropertyMap<AppointmentDto, Appointment> propertyMap2 = new PropertyMap<AppointmentDto, Appointment>() {
            protected void configure() {
                map().setDoctor(source.getDoctor());
                map().setPatient(source.getPatient());
                map().setService(source.getService());
